...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            c_center = Node((self.start.row + self.goal.row)/2, (self.start.col + self.goal.col)/2)
            # print(c_center)

###########################################################
            # Compute rotation matrix from ellipse to world frame - C
            R = np.empty(shape=(3,1))
            R[0] = (self.goal.row - self.start.row) / c_min
            R[1] = (self.goal.col - self.start.col) / c_min
            R[2] = 0
            ...
            M = np.dot(R, I.T)
            # print(M)
            u, s, v_h = np.linalg.svd(M, full_matrices=True)
            v = v_h.T

            det_u = np.linalg.det(u)
            det_v = np.linalg.det(v)

            Cd = np.diag([1, 1, det_u*det_v])
            C = np.dot(u, np.dot(Cd, v_h))

            # Compute diagonal matrix - L
            D1 = c_best/2
            D2 = np.sqrt(c_best**2 - c_min**2)/2
            L = np.diag([D1, D2, D2])

            # Cast a sample from a unit ball - x_ball
            n = 1
            index = 2
            unit = np.random.normal(0, 1, (n, index + 2))
            norm = np.linalg.norm(unit, axis = -1,keepdims = True)
            unit = unit/norm
            x_ball = np.zeros((3, 1))
            x_ball[0:2, :] = np.atleast_2d(unit[0,:index]).T
            
            # Map ball sample to the ellipsoid - x_rand
            x_rand = np.dot(C, np.dot(L, x_ball)) + np.atleast_2d(np.array([c_center.row, c_center.col, 0])).T
