...
class DStar:
    ...
    def process_state(self):
        ...
        if k < n.h:
            for neighbor in nei:
                if (k >= neighbor.h) and (n.h > (neighbor.h + self.cost(neighbor, n))):
                    n.parent = neighbor
                    n.h = neighbor.h + self.cost(neighbor, n)

        # If node k is the same as h (LOWER)
        if k == n.h:
            for neighbor in nei:
                if (neighbor.parent == n and neighbor.h != n.h + self.cost(neighbor, n)) or \
                    (neighbor.parent != n and neighbor.h > n.h + self.cost(neighbor, n)) or (neighbor.tag == "NEW"):
                    neighbor.parent = n
                    self.insert(neighbor, n.h + self.cost(neighbor, n))
        ...
        # Else node k is smaller than h (RAISE)
            ...
            for neighbor in nei:
                ...
                if ((neighbor.parent == n) and (neighbor.h != (n.h + self.cost(neighbor, n)))) or (neighbor.tag == "NEW"):
                    neighbor.parent = n
                    self.insert(neighbor, n.h + self.cost(neighbor, n))

                else:

                    if (neighbor.parent != n) and (neighbor.h > n.h + self.cost(neighbor, n)):
                        self.insert(n, n.h)

                    else:
                        if (neighbor.parent != n) and (n.h > neighbor.h + self.cost(neighbor, n)) and (neighbor.h > k) and (neighbor.tag == "CLOSED"):
    ...
    def prepare_repair(self, node):
        ...
        for neighbor in nei:
            ...
            if neighbor.is_dy_obs == True and neighbor.is_obs == False:
                ...
                other = self.get_neighbors(neighbor)

                for o in other:
                    self.modify_cost(node, o)
